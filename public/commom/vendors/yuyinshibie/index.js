...
// 音频转码worker
let recorderWorker = new Worker('../commom/vendors/yuyinshibie/transformpcm.worker.js')
// 记录处理的缓存音频
let buffer = []
...
class IatRecorder {
  constructor (config) {
    ...
    this.appId = '5d8ae5b6' 
    this.apiKey = '322a162d7054ec7060ad1584cf3a1543'
    this.apiSecret = 'cb655b951c5dd36a616ab1a233189130'
  }
...
class IatTaste {
  ...
  stop () {
    ...
    var resultStr = this.resultTextString.replace(/[\ |\。||\~|\`|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\_|\+|\=|\||\\|\[|\]|\{|\}|\;|\:|\"|\'|\,|\<|\.|\>|\/|\?]/g,""); 
    $('#queryproduct').val(resultStr)
    queryproductChange()
    $('.loadgin_mask').hide()
    $('.loadgin_mask .content p').html('等待顾客支付中...')
    $('.loadgin_mask .content #stop_speak').addClass('hidden')
  }
  ...
  init () {
    ...
    self.resultText = ''
    self.resultTextString = ''
    $('body').on('click','#microphone',function () {
    ...
    $('body').on('click','#stop_speak',function () {
      setTimeout(() => {
        self.reset()
        self.stop()        
      }, 1000);
  ...
  setResult (data) {
    ...
    if (data.pgs === 'apd') {
      this.resultText = this.resultTextString
    }
    resultStr = this.resultText + str
    this.resultTextString = resultStr
    console.log(this.resultTextString)
  }
